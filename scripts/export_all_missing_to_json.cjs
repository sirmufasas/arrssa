...
pages.forEach(p => {
  ...
  trMatches.forEach(m => {
    const s = m.replace(/^tr\(\s*["'`]/, "").replace(/["'`]\s*\)$/, "").replace(/\s+/g, " ").trim();
  ...
  strongMatches.forEach(m => {
    const s = m.replace(/<\/?strong>/g, "").replace(/&amp;/g, "&").replace(/:$/, "").replace(/\s+/g, " ").trim();
    if (s && !existing[s] && s !== "ARSSA" && s !== "ARS S.A.R.L." && s.length > 2) {
      missingStrings.add(s);
    }
  });
  ...
  hMatches.forEach(m => {
    const s = m.replace(/<[^>]+>/g, "").replace(/&amp;/g, "&").replace(/\s+/g, " ").trim();
  ...
  pMatches.forEach(m => {
    const s = m.replace(/<[^>]+>/g, "").replace(/&amp;/g, "&").replace(/&ldquo;|&rdquo;/g, "").replace(/\s+/g, " ").trim();
    if (s && !existing[s] && s !== "ARSSA" && s !== "ARS S.A.R.L." && s.length > 8) {
      missingStrings.add(s);
    }
  });

  // Extract li tags content if pure text
  const liMatches = content.match(/<li[^>]*>([^<{]+)<\/li>/g) || [];
  liMatches.forEach(m => {
    const s = m.replace(/<[^>]+>/g, "").replace(/&amp;/g, "&").replace(/\s+/g, " ").trim();
    if (s && !existing[s] && s !== "ARSSA" && s !== "ARS S.A.R.L." && s.length > 3) {
      missingStrings.add(s);
    }
  });
});

const list = Array.from(missingStrings);
console.log("Found missing strings:", list.length);
fs.writeFileSync(path.join(__dirname, "../all_missing_clean.json"), JSON.stringify(list, null, 2));
