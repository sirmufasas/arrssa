import type { TranslationDictionary } from "../types";

export const sw: TranslationDictionary = {
  "nav": {
    "home": "Mwanzo",
    "about": "Kuhusu Sisi",
    "services": "Huduma",
    "markets": "Masoko",
    "why": "Kwa Nini ARSSA",
    "legacy": "Urithi Wetu",
    "enquiry": "Tuma Ombi",
    "contact": "Wasiliana na ARSSA",
    "appearance": "Mwonekano",
    "divisions": "Idara Zetu",
    "allServices": "Muhtasari wa Huduma Zote",
    "open": "Fungua menyu",
    "close": "Funga menyu",
    "loading": "Inapakia",
    "skip": "Nenda kwenye yaliyomo",
    "selectLanguage": "Chagua Lugha",
    "language": "Lugha"
  },
  "hero": {
    "badge": "Ukanda wa Biashara Afrika Kusini (SADAC) ↔ DRC",
    "headline1": "Suluhisho lako,",
    "headlineGold": "kiganjani mwako",
    "sub": "ARSSA ni njia yako ya haraka ya kuingia katika soko la DRC — mshirika mmoja wa kuaminika anayesimamia uratibu wa biashara, uzingatiaji wa sheria, masoko, usambazaji na biashara ya mipakani kwa sekta za dawa, vipodozi, vyakula na madini. Hakuna makampuni mengi yanayogongana — ni utekelezaji uliopangwa kuanzia siku ya kwanza.",
    "exploreServices": "Chunguza Huduma Zetu",
    "startEnquiry": "Tuma Ombi la Huduma",
    "compliance": "Uzingatiaji wa Sheria",
    "execution": "Utekelezaji wa Mipakani",
    "support": "Usaidizi Kamili",
    "planetCaption": "Afrika inasonga mbele"
  },
  "stats": {
    "coreDivisions": "Idara Kuu",
    "countriesBridged": "Nchi Zilizounganishwa",
    "operatingPresence": "Shughuli za Ukanda",
    "sectorsSupported": "Sekta Muhimu Zinazodhibitiwa"
  },
  "divisionsSection": {
    "badge": "Idara Zetu Maalum",
    "title": "Nguzo Tano za Biashara Iliyounganishwa",
    "subtitle": "Kuunganisha Afrika Kusini (SADAC) na DRC kwa utaalamu thabiti wa kisheria, kiutendaji na usambazaji wa mashinani."
  },
  "divisions": [
    {
      "slug": "business-facilitation",
      "number": "01",
      "title": "Uratibu wa Biashara",
      "shortTitle": "Uratibu wa Biashara",
      "subtitle": "Uwezeshaji, Uzingatiaji wa Sheria, Kuingia Sokoni & Msaada wa Utawala",
      "description": "Uwezeshaji wa biashara, kufuata taratibu za sheria, kuingia sokoni na usaidizi wa kiutawala kwa biashara zinazoingia DRC.",
      "preview": [
        "Kuingia sokoni",
        "Sheria na vibali",
        "Usajili wa kampuni",
        "Nguvu kazi"
      ]
    },
    {
      "slug": "market-growth-distribution",
      "number": "02",
      "title": "Ukuaji wa Soko na Usambazaji",
      "shortTitle": "Ukuaji wa Soko",
      "subtitle": "Masoko, Matangazo, Kuonekana kwa Bidhaa & Usambazaji wa Moja kwa Moja",
      "description": "Mikakati ya masoko, kukuza chapa, kuonekana kwa bidhaa na mtandao thabiti wa usambazaji unaoleta faida halisi.",
      "preview": [
        "Masoko",
        "Matangazo",
        "Usambazaji",
        "Uwezeshaji wa B2B"
      ]
    },
    {
      "slug": "import-export",
      "number": "03",
      "title": "Uagizaji na Usafirishaji Nje",
      "shortTitle": "Uagizaji na Usafirishaji",
      "subtitle": "Uwezeshaji wa Biashara, Forodha, Vifaa na Usafirishaji wa Mipakani",
      "description": "Uwezeshaji wa biashara, vibali vya forodha na usafirishaji salama wa mizigo katika ukanda wa Afrika Kusini (SADAC)–DRC.",
      "preview": [
        "Forodha",
        "Usafirishaji",
        "Ushauri wa biashara",
        "Ugavi wa bidhaa"
      ]
    },
    {
      "slug": "mining",
      "number": "04",
      "title": "Madini na Maliasili",
      "shortTitle": "Madini",
      "subtitle": "Msaada wa Uchimbaji Madini, Ununuzi & Uratibu wa Biashara ya Mipakani",
      "description": "Usaidizi madhubuti wa ununuzi wa vifaa vya migodi, ushirikiano, ugavi wa viwandani na uratibu wa kibiashara katika ukanda wa Afrika Kusini (SADAC)–DRC.",
      "preview": [
        "Msaada wa migodi",
        "Ugavi wa vifaa",
        "Ushirikiano",
        "Uratibu wa biashara"
      ]
    },
    {
      "slug": "maintenance-cleaning",
      "number": "05",
      "title": "Matengenezo, Usafi na Uzingatiaji wa Viwango",
      "shortTitle": "Matengenezo na Usafi",
      "subtitle": "Msaada Maalum wa Uendeshaji kwa Viwanda Vinavyodhibitiwa",
      "description": "Huduma maalum za matengenezo, usafi wa viwandani, kuua vijidudu na kufuata kanuni za viwango katika maeneo ya kazi.",
      "preview": [
        "Matengenezo",
        "Usafi wa viwandani",
        "Kuua vijidudu",
        "Uzingatiaji viwango"
      ]
    }
  ],
  "corridor": {
    "eyebrow": "Faida ya Ukanda",
    "title": "Ukanda wa Afrika Kusini (SADAC) – DRC",
    "lead": "Mshirika Mmoja wa Kuaminika Katika Masoko Mawili Yenye Nguvu",
    "desc": "Kupanua biashara kati ya Afrika Kusini (SADAC) na DRC kunahitaji ufahamu wa kina wa ndani, uzingatiaji thabiti wa sheria na usafirishaji wa kutegemewa. ARSSA inaondoa vikwazo vya mipakani kwa kuunganisha huduma zote chini ya paa moja.",
    "point1Title": "Uelewa wa Mashinani",
    "point1Desc": "Uwepo thabiti wa kiutendaji huko Lubumbashi, Kinshasa, Kolwezi na Johannesburg.",
    "point2Title": "Bila Vikwazo vya Kisheria",
    "point2Desc": "Kushughulikia moja kwa moja vibali vya wizara za DRC, kodi na taratibu za forodha.",
    "point3Title": "Wajibu Mmoja Tu",
    "point3Desc": "Mkataba mmoja wa kibiashara unaojumuisha uratibu, mauzo, usafirishaji na sheria."
  },
  "cta": {
    "headline": "Uko tayari kuingia katika soko la DRC?",
    "text": "ARSSA inatoa msaada kamili wa kiutendaji kwa wafanyabiashara wanaotaka kuanzisha, kuendesha na kukuza biashara katika ukanda wa Afrika Kusini (SADAC)–DRC.",
    "primary": "Tuma Ombi la Huduma",
    "secondary": "Wasiliana na ARSSA"
  },
  "footer": {
    "desc": "ARSSA ni kampuni ya uratibu wa biashara na ukuzaji wa masoko iliyo nchini Afrika Kusini (SADAC), inayosaidia makampuni yanayofanya kazi kati ya Afrika Kusini (SADAC) na Jamhuri ya Kidemokrasia ya Kongo.",
    "quickLinks": "Viungo vya Haraka",
    "services": "Huduma",
    "contact": "Mawasiliano",
    "location": "Afrika Kusini (SADAC) — inahudumia ukanda wa Afrika Kusini (SADAC)–DRC",
    "terms": "Vigezo na Masharti",
    "privacy": "Sera ya Faragha",
    "cookies": "Sera ya Vidakuzi",
    "rights": "© 2026 Agence Rebi Service South Africa. Haki zote zimehifadhiwa.",
    "changeLang": "Badilisha Lugha"
  },
  "modal": {
    "title": "Chagua Lugha Unayopendelea",
    "subtitle": "Chagua lugha yako ya asili ili kufahamu huduma za biashara ya mipakani za ARSSA.",
    "searchPlaceholder": "Tafuta lugha au nchi...",
    "regionalTab": "Ukanda & Kanda (SADAC & DRC)",
    "globalTab": "Washirika wa Kimataifa wa Biashara",
    "continueBtn": "Endelea kwa",
    "preserveNotice": "Utambulisho wa chapa (ARSSA, ARS S.A.R.L.) unabaki vile vile kimataifa."
  },
  "cookie": {
    "text": "Tovuti hii inatumia vidakuzi vichache ili kuhakikisha inafanya kazi vizuri. Tazama sera yetu ya vidakuzi kwa maelezo zaidi.",
    "accept": "Kubali",
    "policy": "Sera ya Vidakuzi"
  },
  "forms": {
    "name": "Jina Kamili",
    "namePlaceholder": "mfano: Juma Rashid",
    "email": "Barua Pepe",
    "emailPlaceholder": "mfano: juma@kampuni.com",
    "phone": "Nambari ya Simu",
    "phonePlaceholder": "mfano: +27 67 279 4750",
    "message": "Ujumbe",
    "messagePlaceholder": "Tueleze kuhusu mradi au ombi lako...",
    "company": "Jina la Kampuni",
    "companyPlaceholder": "mfano: Kampuni ya Viwanda Ltd",
    "industry": "Sekta ya Biashara",
    "service": "Huduma Kuu Inayohitajika",
    "source": "Ulitupataje?",
    "submit": "Wasilisha Ombi",
    "send": "Tuma Ujumbe",
    "submitting": "Inatuma...",
    "successTitle": "Asante Sana",
    "contactSuccessMsg": "Ujumbe wako umepokelewa kwa ufanisi. Wataalamu wetu watawasiliana nawe hivi karibuni.",
    "enquirySuccessMsg": "Ombi lako limepokelewa. Mtaalamu wa ukanda wa ARSSA atakagua taarifa zako na kukujibu haraka.",
    "close": "Funga"
  }
};
